...
- Every interior intersection additionally gets, per arm, a curb → crossing → curb chain linking two ring corners across the street, with the crossing node matched **by geometry** (nearest `TrafficLightIntersection`, same convention as `TrafficNetwork`'s light matching) so `CanCross` can read the real light state via `TrafficNetwork.AxisState`/`IsAxisGreen` (`TrafficLightIntersection.EastWestState`/`NorthSouthState`).
...
- `Build()` wipes `nodes` from scratch every call (Awake in Play, or an explicit re-bake) and rebuilds five node kinds: `Ring`, `Curb`, `Crossing`, and (SPEC 10) `Interior`/`Plaza` — nothing else the generator adds to this graph after `Build()` returns. SPEC 06 removed the one thing that used to (the Point of Interest machinery); see below.
- SPEC 10 (`Interior`/`Plaza`): every non-plaza block **without** a full-block Custom Place (`BlockCell.isPlaza == false` and no `reservedSlots` entry with `slot == -1`) gets a 5-node `Interior` cross (block centre + 4 arm midpoints), connected to that block's own 4 `Ring` midpoint nodes — a shortcut through the block's interior, not a full walkable area. Every `BlockCell.isPlaza == true` block instead gets a dense `Plaza` grid (`CityGeneratorConstants.PlazaGridStep`-spaced, inset by `PlazaGridInset`) covering its footprint, 4-connected to orthogonal neighbours and tied into the same 4 `Ring` midpoints via nearest-node connections. A block with a full-block Custom Place gets neither — same as today, since deciding what counts as an obstacle inside an arbitrary Custom Place prefab is out of scope. Per-block flags (`blockIsPlaza`/`blockIsFullyReserved`, flattened `[bi, bj] -> bool` arrays) are computed by `CityGeneratorPedestrianBuilder.AddNetworkComponent` from `BlockCell`/`reservedSlots` (both Editor-only types `PedestrianNetwork.Build()` must not reference directly) and written via `SerializedObject`, mirroring every other field `AddNetworkComponent`/`AddManagerComponent` wire in.
- A block's `Interior`/`Plaza` nodes tie into its `Ring` midpoints at exactly the same hop-count a same-block `Ring`-only route already has (e.g. the south and north midpoints are 4 hops apart either way around the ring, and also 4 hops via the interior cross), so `FindPath`'s BFS — which resolves same-length ties in favour of whichever edge was built first, always the ring — would never actually route a pedestrian through them. `PickRandomDestination` (below) is what makes them reachable in practice: `Interior`/`Plaza` nodes are valid final destinations, not just potential waypoints.
...
All three levels iterate `network.NodeCount` generically, so `Interior`/`Plaza` nodes (SPEC 10) get exactly the same pruning as `Ring`/`Curb`/`Crossing` with no pipeline changes — a plaza's centerpiece/benches or a quarter-slot Custom Place block any node that lands on them.
...
On top of that it rebuilds a coarse spatial grid every frame and applies a small local separation nudge between nearby agents, plus a stronger player-avoidance nudge — the only peer-interaction mechanic implemented so far (a `PedestrianState.Interacting` value exists on the enum but is never entered, reserved for a future spec). `PedestrianAgent` has **no jam/gridlock mechanics of its own** — crowding only ever shows as this separation nudge, never a stuck agent — so `CityGeneratorConstants.PedestrianCountWarningThreshold` (0.7) is much higher than the vehicle one (0.4). Pedestrians still only ever spawn on `Ring` nodes, but since SPEC 10 the threshold is measured against the network's **total** node count (`PedestrianNetwork.NodeCount`/`CityGeneratorWindow`'s validation-time estimate of it) rather than just the `Ring` count — `Ring` alone stopped representing the block's real walkable capacity once `Interior`/`Plaza` nodes exist. `CityGeneratorPedestrianBuilder.AddManagerComponent` configures every field on the generated instance from `CityGeneratorSettings.crowd` (Pedestrians tab, Crowd card).
