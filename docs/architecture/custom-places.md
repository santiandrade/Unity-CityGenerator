...
- `isPointOfInterest` is saved and shown in the UI but has **no functional effect yet** — a deliberate placeholder for a future minimap/POI system built on top of Custom Places. See `pedestrians.md` for why the *old* POI mechanism was removed instead of reused.
