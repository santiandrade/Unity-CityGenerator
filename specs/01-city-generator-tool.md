...
### Decisiones tomadas después de dar la tool por completa

- **Paradas de bus eliminadas de la tool por completo (2026-08-20).** Se retiraron `props.busStopPrefab`/`props.busStopDensity` de `CityGeneratorSettings`, `BuildBusStops` de `CityGeneratorStreetPropsBuilder`, la referencia por defecto en `CityGeneratorDefaultAssets`, el campo y el conteo del resumen en `CityGeneratorWindow`, y `busStopCount` de `CityBuildSummary`. Se borró también `Assets/Prefabs/Props/BusStop.prefab` y sus cuatro mallas extraídas en `Assets/Meshes/` (`BusStop_BackPanel`, `BusStop_Post_L`, `BusStop_Post_R`, `BusStop_Roof`); no tenía modelos importados ni materiales propios (usaba `MetalDark`, compartido). Decisión del usuario, sin más motivo registrado. El resto de props (farolas, papeleras) no se ve afectado. El código de ejemplo de `PropsSettings` más arriba en este documento **no** se actualizó — es una fotografía de cuando se escribió el spec y ya estaba desincronizado del código real antes de este cambio (p. ej. no refleja la reintroducción de `lampDensity`, ver más abajo).
