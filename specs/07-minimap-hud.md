...
- **`Editor/CityGeneratorMinimapBuilder.cs`** (nuevo) — cuando `minimap.enabled`:
  ...
  - Coloca una cámara ortográfica temporal top-down encuadrando ese bounding box, ocultando temporalmente los grupos `Vehicles`/`Pedestrians` mientras renderiza (ver "decisiones descartadas": `cullingMask` por capa no sirve aquí porque el layer `Vehicle`/`Pedestrian` solo se asigna al collider proxy raíz de cada instancia, nunca a los meshes hijos que dibuja el `Renderer`).
...
- Una capa "Minimap"/"MinimapIgnore" dedicada para excluir manualmente más categorías del snapshot (props, farolas, vegetación): solo se ocultan los grupos `Vehicles`/`Pedestrians` (ver "decisiones descartadas"), los únicos con contenido dinámico que el generador ya gestiona.
...
4. **`Editor/CityGeneratorMinimapBuilder.cs`.** Nuevo builder: calcula el bounding box del grid (`gridWidth`/`gridHeight` × pitch de `CityGeneratorConstants` + margen), oculta temporalmente los grupos `Vehicles`/`Pedestrians` si existen, coloca una cámara ortográfica temporal top-down, renderiza a `RenderTexture` de `minimap.textureResolution`, copia a `Texture2D`, restaura la visibilidad de esos grupos, guarda como asset PNG junto a la escena (mismo path/GUID en Re-Build), destruye la cámara temporal, y rellena un `MinimapData` con la textura, el bounding box y los `PointOfInterestEntry` proyectados desde `customPlaces` donde `isPointOfInterest == true`. Se puede probar de forma aislada invocándolo sobre una escena ya generada con Custom Places.
...
- **El snapshot excluye vehículos/peatones desactivando temporalmente los grupos `Vehicles`/`Pedestrians` durante la captura, no vía `cullingMask` por capa.** Probado primero con `cullingMask` (excluyendo las capas `Vehicle`/`Pedestrian`) tal como describía la versión original de esta spec; descartado en QA manual al comprobar que ambas capas solo se asignan al collider proxy raíz de cada instancia (`CityGeneratorTrafficBuilder`/`CityGeneratorPedestrianBuilder`, por el mismo invariante que deja intacto cualquier collider más profundo en la jerarquía del prefab del usuario) — los meshes hijos con el `Renderer` real se quedan en el layer con el que el prefab fue autorado (típicamente `Default`), así que seguían apareciendo en la textura pese al `cullingMask`. Desactivar los dos grupos enteros durante el `camera.Render()` (restaurados justo después, en un `finally`) evita depender por completo de cómo estén asignadas las capas.
- Una capa "Minimap"/"MinimapIgnore" dedicada para excluir manualmente más categorías del snapshot (props, farolas, vegetación) sigue descartada: `Vehicles`/`Pedestrians` son los únicos grupos con contenido dinámico que el generador gestiona; introducir una capa nueva de propósito general replicaría la complejidad de `EnsureVehicleLayerExists` (gestión de slots de capa, fallback si no hay libres) para un caso de uso que nadie ha pedido todavía.
...
- **`CityGeneratorMinimapBuilder` corre al final del pipeline (tras `TrafficBuilder`), no justo después de `CustomPlaceBuilder`.** Necesita que los grupos `Vehicles`/`Pedestrians` ya tengan su contenido para poder ocultarlos durante la captura — antes de que `TrafficBuilder`/`PedestrianBuilder` los rellenen no hay nada que ocultar — aunque el propio snapshot solo capture geometría estática.
- **El snapshot se aísla moviendo `cityRoot` a un punto lejano del mundo antes de capturar, en vez de ocultar el contenido preexistente ajeno a la ciudad que se está generando.** `Camera.Render()` no está limitado a una escena: dibuja todo lo visible en cualquier escena cargada dentro de su frustum — incluida cualquier otra escena que "Build City in New Scene" deja deliberadamente abierta, y (durante un Re-Build) la ciudad anterior, cuyo root con `CityGeneratorRoot` solo se destruye *después* de que `Assemble` termina, así que sigue ahí, completamente activo, durante toda la captura. Se probaron dos enfoques que parecían correctos y no lo eran: `Camera.scene` (el mecanismo nativo de Unity para esto) no filtra nada cuando la escena aún no se ha guardado — siempre el caso aquí — confirmado con un cubo de prueba; y desactivar (`SetActive(false)`) o cambiar de layer cada root ajeno justo antes de renderizar parecía funcionar en QA superficial pero seguía filtrando vehículos reales en un porcentaje de posiciones, confirmado comparando snapshot contra transforms reales tras varias regeneraciones. La causa, aislada con un repro mínimo (cubo rojo, sin relación con el pipeline): un `Camera.Render()` manual solo refleja un cambio (active, layer o posición, los tres probados) hecho sobre un GameObject que Unity ya ha renderizado alguna vez, si ese cambio ocurrió en una actualización de Editor anterior a la llamada — cambiarlo y renderizar en la misma llamada de script deja la textura con el estado previo al cambio, sea cual sea el mecanismo. Un GameObject recién creado (nunca renderizado antes) no sufre esto, por lo que ocultar `vehiclesGroup`/`pedestriansGroup` (creados un poco antes, en el mismo `Assemble`) sigue funcionando sin cambios. La solución final no toca nada preexistente: mueve `cityRoot` — siempre recién creado en esta llamada — a un desplazamiento aislado (50.000 unidades en X, fuera del alcance de cualquier ciudad plausible y dentro del rango seguro de precisión de `float`), coloca la cámara de captura ahí, y lo restaura después. Verificado con una reproducción extrema (root vacío + ciudad completa activa en su posición real): el snapshot resultante es 100% negro, sin fugas.
- **Fuera de la ciudad, el minimapa pinta el color de fondo del snapshot mediante un shader propio (`DefaultAssets/Shaders/MinimapWindow.shader` + `Materials/MinimapWindow.mat`), en vez de recortar el `uvRect` o ampliar la textura.** Detectado en QA manual sobre un Custom Grid pequeño (3x2 con dos huecos): el `uvRect` que `MinimapHUD` escribe está siempre centrado en el player, así que en cuanto este se acerca a menos de `viewRadiusMeters` de un borde de la ciudad, la ventana sale del rango [0, 1] de la textura. Con el material UI por defecto, el `wrapMode = Clamp` con el que se importa el PNG repite hacia fuera la última fila/columna de píxeles del snapshot, estirando hasta el borde del HUD lo que hubiera en ese borde (en el caso reportado, la hierba de un Empty Block y una franja de acera) — no era un fallo del relleno de bloques vacíos, cuya geometría era correcta. Se descartaron las dos alternativas evidentes: (a) recortar el `uvRect` a [0, 1] obliga a que el mapa deje de desplazarse cerca de los bordes y, con el marcador del player fijo en el centro del HUD, a reposicionarlo a él y a los POI para que sigan coincidiendo con el mundo — rompe la premisa "el player siempre centrado" de esta spec; (b) capturar el snapshot con un margen extra >= `viewRadiusMeters` relleno del color de fondo mantiene el player centrado pero reparte la misma resolución de textura sobre un área mayor (pierde nitidez de la ciudad) y vuelve a fallar en cuanto el player se aleja más de ese margen. El shader es una copia de `UI/Default` (incluidas sus propiedades de stencil, necesarias porque el `RawImage` va enmascarado por el `Mask` circular del HUD) con un único cambio: los fragmentos con UV fuera de [0, 1] se pintan de `_OutOfBoundsColor` en lugar de muestrear la textura. Ese color se mantiene igual al `backgroundColor` de la cámara de captura (un gris neutro, hex `434343`, ajustado tras la primera implementación en negro), de modo que el área exterior a la ciudad se lee como continuación del propio fondo del snapshot.
...
- **Ocultar los grupos `Vehicles`/`Pedestrians` depende de que ya existan (con ese nombre exacto) cuando el builder corre.** Ya mitigado por la decisión de orden en el pipeline (builder al final, después de que `CityGeneratorContentAssembler` los cree vía `GetOrCreateGroup`), pero si en el futuro alguien reordena el pipeline o renombra esos grupos sin darse cuenta de esta dependencia, el snapshot podría volver a incluir vehículos/peatones en el frame de la captura. Mitigación: el criterio de aceptación de "sin vehículos ni peatones visibles en la textura" lo detectaría en QA manual, y el paso 5 del plan fija explícitamente el orden.
