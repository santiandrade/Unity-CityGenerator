...
- **El snapshot desactiva también los root de cualquier otra escena cargada en el Editor, no solo los grupos `Vehicles`/`Pedestrians` de la propia ciudad.** `Camera.Render()` no está limitado a una escena: dibuja todo lo visible en cualquier escena cargada dentro de su frustum. `"Build City in New Scene"` deja deliberadamente abierta la escena actual (su propio tooltip lo dice), así que sin este aislamiento, generar una ciudad nueva con otra ya abierta filtraba los edificios/vehículos/peatones de esa otra escena al snapshot nuevo — encontrado en QA comparando la posición en píxeles de un vehículo del snapshot contra la transform real de un vehículo de la otra escena, coincidencia exacta. `Camera.scene` (el mecanismo nativo de Unity para esto, usado internamente por Prefab Mode) se probó primero y se descartó: no filtra en absoluto cuando la escena aún no se ha guardado (nombre/ruta vacíos) — verificado de forma directa con un cubo de prueba — y la escena de `cityRoot` está siempre sin guardar en este punto del pipeline (una escena nueva solo se guarda después de que `Assemble` termina). Desactivar los root de cada escena ajena (restaurados en un `finally`) no depende del estado de guardado.
