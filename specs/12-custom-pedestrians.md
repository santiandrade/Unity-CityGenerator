...
- **Picker visual por entrada**: un nuevo modo de `CityGeneratorGridPreview` que, en vez de manzanas/cuadrantes, dibuja el grid de la ciudad con los nodos peatonales (Ring, Interior, Curb, Crossing) como puntos, coloreados por tipo. El usuario hace clic para añadir/quitar nodos a la selección de esa entrada; un clic solo se acepta si el nodo está directamente conectado (por el grafo real) a algún nodo ya seleccionado, salvo el primer clic (libre). Cada arista real entre dos nodos seleccionados se dibuja en amarillo. Quitar un nodo que partiría la selección en varios grupos conexos deja solo el grupo conexo más grande tras la deselección.
...
4. **Modo "node graph" de `CityGeneratorGridPreview`.** Nuevo modo que consume el `CityGeneratorPedestrianPreview` del paso 3: dibuja el grid de fondo, un punto por nodo (color por `PedestrianNodeKind`), resalta los nodos seleccionados y dibuja en amarillo cada arista real entre dos nodos seleccionados. Clic en un nodo no seleccionado lo añade solo si es vecino directo de algún nodo ya seleccionado (o si la selección está vacía); clic en un nodo seleccionado lo quita, y si eso parte el resto en varios componentes conexos, se conserva solo el mayor. Manual test: en una ventana de prueba con una entrada `CustomPedestrianEntry` puesta a mano, trazar una red pequeña a golpe de clic y verificar visualmente la línea amarilla y las reglas de adyacencia/desconexión.
...
- [x] La card "Custom Pedestrians" (tab Pedestrians) permite añadir/quitar entradas, asignar título/prefab/count, y trazar la red de nodos con el picker visual (clic para añadir/quitar, solo nodos directamente conectados a la selección actual salvo el primero, línea amarilla por cada arista real entre nodos seleccionados).
- [x] Quitar un nodo puente de la selección conserva solo el componente conexo más grande restante, sin dejar la selección en un estado desconectado.
- [x] El picker muestra los 4 tipos de nodo (Ring, Interior, Curb, Crossing) correctamente posicionados **antes** de generar la ciudad por primera vez en la escena, coincidiendo exactamente con el grafo que produce una generación real con los mismos settings.
