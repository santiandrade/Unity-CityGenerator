...
- **Fórmula de rotación** (pitch, eje X, grados): `pitch = hourOfDay / 24f * 360f - 90f`. Con esto, hora 6 (amanecer) = 0° (luz en el horizonte), hora 12 (mediodía) = 90° (luz apuntando hacia abajo, sol en el cenit), hora 18 (atardecer) = 180° (horizonte opuesto), hora 0/24 (medianoche) = -90° (luz apuntando hacia arriba, sol "por debajo"). ~~El yaw y roll con los que se creó la luz (por defecto -30° y 0°, los mismos de hoy) se mantienen fijos; solo el pitch varía.~~ **Revisado (2026-08-28):** el yaw ya no se deja "como estaba" — se fuerza siempre a **-90°** (roll a 0°), tanto al crear la luz como en cada Re-Build, incluso si la luz ya existía con otro yaw. Motivo: con la cámara del snapshot del minimapa mirando hacia abajo sin yaw (`Euler(90,0,0)`), minimap-derecha = mundo +X y minimap-arriba = mundo +Z; como en el minimapa la derecha es Este, un yaw de -90° hace que, con la fórmula de pitch de arriba, el sol salga exactamente por el Este (hora 6) y se ponga exactamente por el Oeste (hora 18) — un jugador mirando al Este ve amanecer, mirando al Oeste ve atardecer. El -30° original era un valor puramente estético sin relación con los ejes del minimapa. **Revisado de nuevo (2026-08-28):** el yaw pasa de -90° a **-110°**. Se mantiene todo el razonamiento anterior (se sigue forzando en cada build/re-build, y el eje sigue siendo aproximadamente este-oeste, así que el minimapa se lee igual), pero desplazado 20°: el sol sale por el este-noreste y se pone por el oeste-suroeste, de modo que las sombras no caen exactamente sobre el eje de las calles en una rejilla cuya geometría es toda paralela.
- `baseRotation` guarda el yaw/roll que usa `ApplySun` para el pitch. Ya no se captura una única vez de forma pasiva: `CityGeneratorSceneBuilder` llama a `DayNightCycle.SetBaseRotation(Quaternion.Euler(0, CityGeneratorSceneBuilder.DirectionalLightYaw, 0))` en cada build/re-build (`ConfigureDayNightCycle`), sobrescribiendo el yaw/roll aunque el componente ya existiera con otro valor. Esa constante (-110° desde 2026-08-28, ver nota anterior) es también la que lee la luz neutra del snapshot del minimapa, en vez de duplicar el número.
...
- [x] ~~Generar una ciudad nueva con `dayNight.enabled = false` deja la Directional Light exactamente como hoy (sin `DayNightCycle`, rotación estática `Euler(50,-30,0)`).~~ **Revisado:** ver nota posterior — `DayNightCycle` ahora se añade siempre y `Start Hour` se aplica siempre, incluso con `enabled = false`.
...
- **Revisión (2026-08-28): `Start Hour` se aplica siempre, aunque `Enabled` esté desactivado.** El comportamiento original ("con `enabled = false`, la luz se queda en su rotación estática de siempre, sin `DayNightCycle`") resultaba confuso: ajustar `Start Hour` con el ciclo desactivado no tenía ningún efecto visible. Decisión explícita del usuario. Cambio: `ConfigureDayNightCycle` ya no destruye el componente cuando `enabled` es `false` — lo añade/mantiene siempre y llama a `ApplySun(startHour)` incondicionalmente; `dayNight.enabled` pasa a mapearse solo sobre el `enabled` propio de `MonoBehaviour` del componente, que Unity usa para saltarse `Update()` en Behaviours desactivados. Así la luz siempre queda orientada/coloreada según `Start Hour` (en Editor y al entrar en Play), pero solo avanza automáticamente la hora simulada cuando `Enabled` está activo. No rompe el resto de invariantes: `RebuildInActiveScene` sigue sin tocar base rotation/shadows más allá del pitch del ciclo, y sigue recreando la luz si fue borrada manualmente.
