...
- **The Custom Pedestrians graph preview's root is inactive and carries its own `CityGeneratorRoot`.** It instantiates the user's real Traffic Light Prefab, and `HideFlags.HideAndDontSave` only hides an object from the Hierarchy — it still renders and still registers colliders, so an active root drew a grid of traffic lights across the Scene view on every edit to Grid Width/Height with nothing in the Hierarchy to explain them. The marker is what keeps `PedestrianNetwork` scoping its `TrafficLightIntersection` search to the preview instead of falling back to a scene-wide search that sees nothing under an inactive root (and that used to cross-match a real generated city's intersections). ([pedestrians](docs/architecture/pedestrians.md))
